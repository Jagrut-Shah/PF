...
const WhatsAppIcon = () => (
  <svg viewBox="0 0 24 24" fill="#128C7E" className="w-4 h-4 shrink-0 fill-[#128C7E] text-[#128C7E]" aria-hidden="true">
...
export default function ContactHelpBlock() {
  ...
  return (
    <section className="mt-12 pt-10 border-t border-[#E6E2DA] text-center max-w-xl mx-auto">
      ...
      <div className="mt-8 p-4 bg-[#FAF7F2] border border-[#ECE7DE] rounded-xl text-left font-sans text-xs text-[#555] space-y-1">
        <div className="flex items-center justify-between text-[10px] uppercase font-bold text-[#9E6E24] tracking-wider mb-1">
          <span>{DEMO_COMPANY_INFO.brandName}</span>
          <span className="bg-[#F5EFE0] px-1.5 py-0.5 rounded">DEMO CONTACT</span>
        </div>
        <p className="text-[#171717] font-medium">{DEMO_COMPANY_INFO.formattedAddress}</p>
        <p>Customer Care: <a href={`mailto:${DEMO_COMPANY_INFO.supportEmail}`} className="underline text-[#171717] hover:text-[#CFA838]">{DEMO_COMPANY_INFO.supportEmail}</a> <span className="text-[10px] text-[#77736B]">(DEMO)</span></p>
        <p>Support Hours: {DEMO_COMPANY_INFO.customerCareHours}</p>
      </div>
    </section>
  );
}
