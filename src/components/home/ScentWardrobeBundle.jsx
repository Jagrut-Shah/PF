 ...
 * Deep Black #0B0B0B environment, Deep Red #8F1018 header badge, Soft Black #121212 cards, Signature Red #B4171E accents.
 */
  ...
  return (
    <section className="py-12 sm:py-16 bg-[#0B0B0B] text-[#F5F2EE] border-t border-b border-white/10 relative overflow-hidden">
      <MainContainer>
        ...
        {/* 3 Curated Bundle Preset Cards — Soft Black #121212 Cards with Deep Red Accent Highlights */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
          {BUNDLE_PRESETS.map((preset) => (
            <Link
              key={preset.id}
              to={`/wardrobe/${preset.id}`}
              className="p-6 rounded-2xl border border-white/10 bg-[#121212] hover:border-[#B4171E]/60 active:scale-[0.98] transition-all duration-200 cursor-pointer flex flex-col justify-between group shadow-md hover:shadow-xl relative overflow-hidden"
            >
