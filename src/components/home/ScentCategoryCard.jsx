 ...
 * Black background #120E0D + Very Dark Cream #DAC29F title.
 */
export default function ScentCategoryCard({ category }) {
  ...
  return (
    <Link
      to={route}
      className="group block relative w-full rounded-xl overflow-hidden border border-[#3D2E2A] bg-[#120E0D] transition-all duration-300 hover:border-[#DAC29F]/50 hover:-translate-y-1 shadow-md select-none"
    ...
    >
      <div className="relative aspect-[1/0.92] sm:aspect-[4/3.6] md:aspect-[4/3.2] w-full overflow-hidden bg-[#120E0D]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 flex flex-col justify-end text-left z-10">
          <h3 className="font-bodoni text-[18px] sm:text-[20px] md:text-[22px] font-medium text-[#DAC29F] tracking-[-0.01em] leading-tight group-hover:text-[#E5D7C3] transition-colors">
            {title}
          </h3>

          {descriptorsStr && (
            <p className="hidden md:block font-manrope text-[12px] text-[#DAC29F]/70 font-normal leading-tight mt-1 line-clamp-1">
              {descriptorsStr}
            </p>
          )}

          <div className="mt-1.5 inline-flex items-center gap-1.5 text-[12px] font-manrope font-semibold text-[#DAC29F]/80 group-hover:text-[#E5D7C3] transition-colors">
            <span>Explore</span>
